...
function parseRow(str) {
    const halfes = str.split(' -> ');
    const curr = halfes[0].split(" ");
    const weight = halfes[0].replace(/[^\d]/g, '');
    const children = halfes[1] !== undefined ? halfes[1].split(", ") : [];
    return {name: curr[0], weight: parseInt(weight), children: children };
}

function findRoot(tree) {

    const nodes = tree.reduce((acc, curr) => acc.concat([curr.name]), []);
    const children = tree.reduce((acc, curr) => acc.concat(curr.children), []);
    const rootnode = nodes.reduce((acc, curr) => children.indexOf(curr) === -1 ? curr : acc, "");
    return rootnode;
}

// function getChildnodes(ptree, node) {
//     const children = node.children.reduce((acc, curr) => {

//     }, {});
// }

function ptreetohash(ptree) {
    const hash = ptree.reduce((acc, curr) => {
        // console.log(curr);
        acc[curr.name] = curr;
        return acc;
    }, {});
...
function getChildren(ptree, rootname) {
    const node = ptree[rootname];
    const newNode = { name: node.name, children: [], totalWeight: parseInt(node.weight), weight: parseInt(node.weight) };
    // console.log("name:", rootname, node.name);
    if (node.children.length > 0) {
        for (const child of node.children) {
            let childNodes = getChildren(ptree, child);
            // console.log(">", rootname, childNodes);
            newNode.totalWeight += childNodes.totalWeight;
            newNode.children.push(childNodes);
        }
    } else {
        newNode.totalWeight = node.weight;
    }
    return newNode;
    // const hash = ptree.reduce((acc, curr) => {
    //     // console.log(curr);
    ...
    // ptree[rootname].children
    // const tree = {};

    // return tree;
}

const pseudotree = data
    .split("\n")
    .reduce((acc, curr) => (curr !== "" ? acc.concat([curr]) : acc), [])
    .reduce((acc, curr) => acc.concat([parseRow(curr)]), []);

const rootname = findRoot(pseudotree);
// RUN ONE
// -------
// console.log(rootname);
// RUN TWO
// --------
const hashes = ptreetohash(pseudotree);
const tree = getChildren(hashes, rootname);
console.log(tree.children[1].children[4]);
