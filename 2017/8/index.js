...
for (const instruction of data) {
    const statarr = instruction.split(" if ")[0].split(" ");
    const condarr = instruction.split(" if ")[1].split(" ");

    if (reg[condarr[0]] === undefined) { reg[condarr[0]] = 0;}
    if (reg[statarr[0]] === undefined) { reg[statarr[0]] = 0;}

    let condres;
    switch (condarr[1]) {
        case '>':
            condres = reg[condarr[0]] > condarr[2];
            break;
        case '<':
            condres = reg[condarr[0]] < condarr[2];
            break;
    ...
    if (condres) {
        switch (statarr[1]) {
            case "inc":
                reg[statarr[0]] += parseInt(statarr[2]);
                break;
            case "dec":
                reg[statarr[0]] -= parseInt(statarr[2]);
                break;
    ...
    // find global max
    gm = Math.max(gm, Object.keys(reg).reduce((acc, curr) => { return reg[curr] > acc ? reg[curr] : acc; }, 0));

    // console.log(reg, condarr, statarr, condres);
}

let m = 0;
// for(const k of Object.keys(reg)) { m = Math.max(reg[k], m); if (reg[k] === m) { console.log(k, m);} }
m = Object.keys(reg).reduce((acc, curr) => { return reg[curr] > acc ? reg[curr] : acc; }, 0);
console.log("---------");
console.log("max:", m);
console.log("gmax:", gm);
