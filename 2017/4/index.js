...
for (const row of input.split('\n')) {
    ...
    for (const word of row.split(' ')) {
        ...
        if (mem[word] !== undefined) { valid=false; break; } else { mem[word] = true; }
    }
    sum += valid ? 1 : 0;
}
console.log("sum", sum);

function hasAnagram(words) {
    let mem = {};
    for (const word of words) {
        const sword = word.split("").sort().join("");
        console.log(word, "=>", sword);
        if (mem[sword] !== undefined) { console.log("NOT OK"); return true; }
        mem[sword] = true;
    }
    console.log("OK");
    return false;
